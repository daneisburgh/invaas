...
def download_all_documents(doctype=None, dirpath=None):
    ...
    if downloaded_files == False:
        ...
        if counter == 1:
            print("Done - wrote {} file to {}".format(counter, os.path.abspath(directory)), file=get_output())
        else:
            print("Done - wrote {} files to {}".format(counter, os.path.abspath(directory)), file=get_output())
    ...
    for item in positions_data:
        ...
        try:
            ...
            if float(item["average_buy_price"]) == 0.0:
                ...
                percent_change = (
                    (float(price) - float(item["average_buy_price"])) * 100 / float(item["average_buy_price"])
                )
            ...
            holdings[symbol].update({"intraday_percent_change": "{0:.2f}".format(intraday_percent_change)})
